...
// Drag & drop
const dragging = ref<string | null>(null); // device id
const dragOverGroup = ref<string | null>(null); // group id or "ungrouped"
...
const filteredSections = computed(() => {
    ...
    return sections.value
        .map((s) => {
            const groupName = (s.group?.name ?? "Ungrouped").toLowerCase();
            const groupMatches = groupName.includes(q);

            const matchedDevices = groupMatches
                ? s.devices // group name matched → show all devices
...
function isDragTarget(s: GroupSection) {
    ...
    return dragOverGroup.value === key && dragging.value !== null;
}
</script>
...
<template>
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex items-center gap-0.5 shrink-0">
                            ...
                            <Button
                                ...
                                :aria-label="`Add device to ${section.group?.name ?? 'ungrouped'}`"
                            >
                                <RiAddLine class="size-3.5" />
                            </Button>
                        </div>
                    ...
                    >
                        ...
                        <div
                            ...
                            :class="[
                                'group flex cursor-pointer rounded-lg border border-border bg-card transition-all',
                        ...
                        >
                            ...
                            >
                                ...
                                />
                                <!-- drag handle -->
                                <div class="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground cursor-grab active:cursor-grabbing">
                                    <RiDraggable class="size-3.5" />
                                </div>
                                <!-- status dot -->
                                <div class="absolute top-2 right-2">
                                    <span
                                        v-if="device.online === null"
                                        class="block size-2 rounded-full bg-muted-foreground/30 animate-pulse"
                                    />
                                    <span
                                        v-else
                                        :class="[
                            ...
                            <!-- List: icon -->
                            <div v-if="viewMode === 'list'" class="relative shrink-0">
                                <component
                                    :is="getPluginIcon(device.plugin_id)"
                            ...
                            <div v-if="viewMode === 'list'" class="flex items-center gap-2 shrink-0">
                                <span
                                    v-if="device.online === null"
                                    class="block size-1.5 rounded-full bg-muted-foreground/30 animate-pulse"
                                />
                                <span
                                    v-else
                                    :class="[
                                ...
                                />
                                <div class="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground cursor-grab active:cursor-grabbing">
                                    <RiDraggable class="size-3.5" />
                                </div>
                            </div>
                        </div>
            ...
            >
                ...
                <div class="relative z-10 w-full max-w-sm rounded-xl border border-border bg-card shadow-xl px-6 py-5 mx-4">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <Button variant="ghost" size="icon-sm" @click="showNewGroup = false">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="flex gap-2 justify-end mt-1">
                            <Button variant="outline" size="sm" @click="showNewGroup = false">Cancel</Button>
                            <Button size="sm" :disabled="savingGroup || !newGroupName.trim()" @click="createGroup" class="gap-1.5">
                                <RiLoader4Line v-if="savingGroup" class="size-3.5 animate-spin" />
    ...
    <Teleport to="body">
        <Transition name="fade">
            <div
                v-if="showNewDevice"
                class="fixed inset-0 z-50 flex items-center justify-center"
                @click.self="showNewDevice = false"
            >
                <div class="absolute inset-0 bg-black/50" />
                <div class="relative z-10 w-full max-w-sm rounded-xl border border-border bg-card shadow-xl px-6 py-5 mx-4">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p class="text-xs text-muted-foreground mt-0.5">
                                {{
                                    newDeviceTargetGroup
                                        ? `Group: ${groups.find(g => g.id === newDeviceTargetGroup)?.name ?? '—'}`
                                        : 'No group'
                                }}
                            </p>
                        </div>
                        <Button variant="ghost" size="icon-sm" @click="showNewDevice = false">
                            <RiCloseLine class="size-3.5" />
                        </Button>
                    </div>
                    <div class="flex flex-col gap-3">
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-medium">Name</label>
                            <Input v-model="newDeviceName" placeholder="e.g. Bambu A1 #1" />
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-medium">Plugin</label>
                            <select
                                v-model="newDevicePlugin"
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <select
                                v-model="newDeviceCredential"
                                class="bg-input/20 dark:bg-input/30 border-input focus-visible:border-ring focus-visible:ring-ring/30 h-7 rounded-md border px-2 py-0.5 text-xs w-full outline-none"
                            >
                                <option v-for="c in credentials" :key="c.id" :value="c.id">{{ c.name }}</option>
                                <option v-if="!credentials.length" value="" disabled>No credentials</option>
                            </select>
                        </div>
                        <div
                            v-if="deviceError"
                        ...
                        <div class="flex gap-2 justify-end mt-1">
                            <Button variant="outline" size="sm" @click="showNewDevice = false">Cancel</Button>
                            <Button
                                size="sm"
                                :disabled="savingDevice || !newDeviceName.trim() || !newDevicePlugin || !newDeviceCredential"
                                @click="createDevice"
                                class="gap-1.5"
                            >
    ...
    <Teleport to="body">
        <Transition name="fade">
            <div
                v-if="editGroup"
                class="fixed inset-0 z-50 flex items-center justify-center"
                @click.self="closeEditGroup"
            >
                <div class="absolute inset-0 bg-black/50" @click="closeEditGroup" />
                <div class="relative z-10 w-full max-w-sm rounded-xl border border-border bg-card shadow-xl px-6 py-5 mx-4">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-sm font-semibold">Edit group</h2>
                        <Button variant="ghost" size="icon-sm" @click="closeEditGroup">
                            <RiCloseLine class="size-3.5" />
                        </Button>
                    </div>
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <div class="flex gap-2">
                                ...
                                <Button
                                    size="sm"
                                    :disabled="savingGroupName || !editGroupName.trim() || editGroupName === editGroup.name"
                                    @click="saveGroupName"
                                    class="gap-1.5 shrink-0"
                                >
                                    <RiLoader4Line v-if="savingGroupName" class="size-3.5 animate-spin" />
                                    <RiCheckLine v-else class="size-3.5" />
                                    Save
                                </Button>
                            </div>
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <div v-if="loadingToken" class="flex items-center gap-2 text-xs text-muted-foreground">
                                <RiLoader4Line class="size-3.5 animate-spin" />
                                Loading…
                            </div>
                            <div v-else-if="groupToken" class="flex flex-col gap-2">
                                <div class="rounded-md border border-border bg-muted/30 px-2.5 py-1.5 font-mono text-[10px] text-muted-foreground break-all">
                                    {{ groupToken }}
                                </div>
                                <div class="flex gap-2">
                                    <Button variant="outline" size="sm" class="gap-1.5 flex-1" @click="copyToken">
                                        <RiCheckLine v-if="tokenCopied" class="size-3.5 text-emerald-500" />
                                        <RiFileCopyLine v-else class="size-3.5" />
                                        {{ tokenCopied ? 'Copied!' : 'Copy' }}
                                    </Button>
                                    <Button variant="outline" size="sm" class="gap-1.5 flex-1" :disabled="rotatingToken" @click="rotateToken">
                                        <RiLoader4Line v-if="rotatingToken" class="size-3.5 animate-spin" />
                        ...
                        <div class="flex gap-2 justify-between pt-1 border-t border-border">
                            ...
                            <Button variant="outline" size="sm" @click="closeEditGroup">Close</Button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <Teleport to="body">
        <Transition name="fade">
            ...
            >
                <div class="absolute inset-0 bg-black/50" @click="confirmDeleteGroup = false" />
                <div class="relative z-10 w-full max-w-xs rounded-xl border border-border bg-card shadow-xl px-6 py-5 mx-4">
                    ...
                    <p class="text-xs text-muted-foreground mt-1.5">
                        <span class="font-medium text-foreground">{{ editGroup?.name }}</span>
                        will be deleted. Devices in this group will become ungrouped.
                    </p>
                    <div class="flex gap-2 justify-end mt-4">
                        <Button variant="outline" size="sm" @click="confirmDeleteGroup = false">Cancel</Button>
                        <Button
                            variant="destructive"
                            size="sm"
                            class="gap-1.5"
                            ...
                            @click="deleteGroup"
                        >
                            <RiLoader4Line v-if="deletingGroup" class="size-3.5 animate-spin" />

...
const devices = ref<{ id: string; name: string; credential_id: string; plugin_id: string }[]>([]);
const plugins = ref<PluginMeta[]>([]);
const loading = ref(true);
const error = ref<string | null>(null);

// Templates built from plugin _credentials
const templates = ref<{ id: string; name: string; fields: { field: string; type: string; label: string; required?: boolean }[] }[]>([]);
...
async function load() {
    ...
    try {
        ...
        // Build templates from plugin _credentials fields
        const templateList: typeof templates.value = [];
        for (const plugin of plugs.items) {
...
async function saveCredential(name: string, data: Record<string, string>) {
    ...
    saveError.value = null;
    try {
        const updated = await api.credentials.update(selected.value.id, { name, data });
...
async function deleteCredential() {
    ...
    try {
        ...
        credentials.value = credentials.value.filter((c) => c.id !== selected.value!.id);
        closeEdit();
    } catch (e) {
...
<template>
    <div class="flex flex-col h-full">
        ...
        <div class="flex-1 overflow-auto px-6 py-4">
            ...
            <div v-else-if="credentials.length === 0" class="flex flex-col items-center justify-center py-20 text-center gap-3">
                <RiKeyLine class="size-10 text-muted-foreground/30" />
                <div>
                    <p class="text-xs font-medium text-foreground">No credentials yet</p>
                    <p class="text-xs text-muted-foreground mt-0.5">Add credentials to connect your devices</p>
                </div>
            ...
            <div v-else class="flex flex-col divide-y divide-border rounded-lg border border-border overflow-hidden">
                <button
                    ...
                    @click="openEdit(cred)"
                >
                    <div class="shrink-0 flex items-center justify-center size-7 rounded-md bg-muted/60 text-muted-foreground">
                        <RiKeyLine class="size-3.5" />
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-medium text-foreground truncate">{{ cred.name }}</p>
                        <p class="text-[10px] text-muted-foreground mt-0.5 truncate">
                            <template v-if="devicesUsing(cred.id).length > 0">
                                ...
                                <span v-for="(dev, i) in devicesUsing(cred.id)" :key="dev.id">
                                    {{ dev.name }}<span v-if="i < devicesUsing(cred.id).length - 1">,</span>
                                </span>
                            </template>
                    ...
                    <div class="shrink-0 flex items-center gap-2">
                        <span class="text-[10px] text-muted-foreground tabular-nums">
                            {{ Object.keys(cred.data).length }} field{{ Object.keys(cred.data).length !== 1 ? "s" : "" }}
                        </span>
                        <RiPencilLine class="size-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                </button>
            </div>
        </div>

...
export const api = {
  // ── Auth (public endpoints have auth=false) ──────────────────────────────────
  auth: {
    setupStatus:  ()                     => get<SetupStatus>("/auth/setup/status", false),
    setup:        (body: SetupRequest)   => post<TokenResponse>("/auth/setup", body, false),
    login:        (body: LoginRequest)   => post<TokenResponse>("/auth/login", body, false),
    me:           ()                     => get<User>("/auth/me"),
  },
  ...
  // ── Device actions ────────────────────────────────────────────────────────────
  actions: {
    send:    (deviceId: string, action: string, body: Record<string, unknown> = {}) =>
               post<unknown>(`/devices/${deviceId}/send/${action}`, body),
    request: (deviceId: string, action: string, path?: string) =>
               get<unknown>(`/devices/${deviceId}/request/${action}${path ? `?path=${encodeURIComponent(path)}` : ""}`),
  },
  ...
  credentials: {
    ...
    update: (id: string, b: CredentialUpdate) => patch<Credential>(`/credentials/${id}`, b),
  ...
  groups: {
    ...
    rotateToken: (id: string)              => post<GroupWithToken>(`/groups/${id}/rotate-token`),
  ...
  plugins: {
    ...
    reloadAll: () => post<{ message: string; protocols: number; devices: number }>("/plugins/reload"),
  },
